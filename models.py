...
def alexnet_fcn_voc(data_input):
    ...
    shape = data_input.get_shape().as_list()
    #shape = [1, 200, 769,3]
    up_score = tflearn.layers.conv.upscore_layer(fr, num_classes = 21, shape = shape, kernel_size = 63, strides = 16 )
    #up_score = tflearn.layers.conv.upscore_layer(fr, num_classes = 21, kernel_size = 63, strides = 16 )
    return up_score 
